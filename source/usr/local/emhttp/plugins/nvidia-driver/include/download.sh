...
if [ "${SET_DRV_V}" == "latest_nos" ]; then
  ...
  LAT_NOS_AVAIL="$(wget -qO- https://api.github.com/repos/mricharz/unraid-nvidia-driver/releases/tags/${KERNEL_V} | jq -r '.assets[].name' | grep -E -v '\.md5$' | grep "${PACKAGE}" | awk -F "-" '{print $2}' | sort -V | tail -1)"
else
  export PACKAGE="nvidia"
  export DRIVER_AVAIL="$(wget -qO- https://api.github.com/repos/mricharz/unraid-nvidia-driver/releases/tags/${KERNEL_V} | jq -r '.assets[].name' | grep -E ${PACKAGE} | grep -E -v '\.md5$' | sort -V)"
  export BRANCHES="$(wget -qO- https://raw.githubusercontent.com/mricharz/unraid-nvidia-driver/master/versions.json | jq -r '.branches')"
fi
export DL_URL="https://github.com/mricharz/unraid-nvidia-driver/releases/download/${KERNEL_V}"
