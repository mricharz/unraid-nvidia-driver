...
if [ -f /tmp/nvidia_driver ]; then
  ...
  if [ $DIFF -gt $CHK_TIMEOUT ]; then
    DRIVERS="$(wget -qO- https://api.github.com/repos/mricharz/unraid-nvidia-driver/releases/tags/${KERNEL_V} | jq -r '.assets[].name' | grep -E -v '\.md5$' | sort -V)"
    ...
    echo -n "$(wget -qO- https://api.github.com/repos/mricharz/unraid-nvidia-driver/releases/tags/${KERNEL_V} | jq -r '.assets[].name' | grep "${PACKAGE}" | grep -E -v '\.md5$' | awk -F "-" '{print $2}' | sort -V | tail -10)" > /tmp/nvidia_driver
...
if [ -f /tmp/nvidia_branches ]; then
  ...
  DIFF=$(expr $CURENTTIME - $FILETIME)
  if [ $DIFF -gt $CHK_TIMEOUT ]; then
    echo -n "$(wget -q -N -O /tmp/nvidia_branches https://raw.githubusercontent.com/mricharz/unraid-nvidia-driver/master/versions.json)"
